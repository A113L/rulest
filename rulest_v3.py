...
# ----------------------------------------------------------------------
#
# minimize_by_signature() above answers "which rules are functionally
# distinct?" — it says nothing about which of those distinct rules are
# actually worth keeping for a given budget. Sorting by raw GPU frequency
# (the historical default in main()) picks rules that individually recover
...
# have spent better.
#
# The functions below implement the "select by marginal coverage, not by
# summed score" idea: build the real coverage matrix C(r) = {target words
# recovered by rule r} against the actual --target-hours target wordlist
# (not the tiny BUILTIN_PROBES set used for signature dedup), then run a
# lazy greedy (CELF) set-cover selection over it. This reuses
# GeneticRuleEvolver's existing GPU dispatch (single-GPU or MultiGPU
# fan-out via evaluate_population) instead of duplicating kernel-launch
# code — it's the same "chains x words -> hit set" computation the GA
# already does per-generation, just run once over the whole candidate pool.
...
def evaluate_full_coverage(gpu_engine, base_words, target_words, rules, max_depth, batch_size=4000):
    ...
    # which is the actual cause of the x2-x10 slowdown / RSS growth at
    # 20M-word targets). Returned hit indices are indices into base_words.
...
def select_rules_by_marginal_coverage(coverage, counts, n_target_words, budget=None, cost_alpha=0.0):
    ...
    `cost_alpha`: gain(r) = new_recovery(r) / depth(r)**cost_alpha. 0 = pure
                marginal-coverage greedy (Experiment C). >0 approximates
                Experiment D (greedy + cost), favoring shorter/cheaper chains
                among rules with similar marginal gain.

    Coverage sets are tracked as a single numpy boolean mask over base_words
    (updated in place with fancy indexing) rather than Python
    set unions — with 150k-1.7M candidates this avoids repeated hashing and
    boxed-int set churn, matching the same memory rationale as switching
...
// --exact-recovery verification kernel. Applies the FULL rule chain
// cseq[pair_chain_idx[gid]] to word pair_word_idx[gid] using the SAME
// per-token apply() loop as find_rule_chains_gpu above (unmodified — no new
// rule semantics here, just chain application), and unconditionally writes
// the resulting string + length to host-visible buffers. No bloom test, no
// atomic counter: meant to be called only on the small (chain, word) pairs
// already flagged as bloom hits during selection, so every gid is a real
// result the host wants to see, not a candidate that might not match.
__kernel void verify_pairs_gpu(
    __global const unsigned char *bw, __global const int *bo, __global const int *bl,
    __global const unsigned char *rs, __global const int *ro, __global const int *rl,
    __global const int *cseq, __global const int *cdep,
    __global const int *pair_word_idx, __global const int *pair_chain_idx,
    const int npairs, const int mcd,
    __global unsigned char *out, __global int *out_len)
{{
    int gid = get_global_id(0);
    if (gid >= npairs) return;
    int wi = pair_word_idx[gid], ci = pair_chain_idx[gid];
    unsigned char cur[MAX_OUTPUT_LEN], tmp[MAX_OUTPUT_LEN], rr[MAX_RULE_LEN];
    int wl = bl[wi]; for (int i = 0; i < wl; i++) cur[i] = bw[bo[wi] + i]; cur[wl] = '\0';
    int cl_ = wl, dep = cdep[ci];
    for (int d = 0; d < dep; d++) {{
        int ri = cseq[ci * mcd + d]; if (ri < 0) break;
        int rlen = rl[ri]; for (int i = 0; i < rlen; i++) rr[i] = rs[ro[ri] + i]; rr[rlen] = '\0';
        int nl; apply(rr, rlen, cur, cl_, tmp, &nl);
    ...
    if (cl_ > MAX_OUTPUT_LEN) cl_ = MAX_OUTPUT_LEN;
    out_len[gid] = cl_;
    __global unsigned char *p = out + (long)gid * MAX_OUTPUT_LEN;
    for (int i = 0; i < cl_; i++) p[i] = cur[i];
}}
"""
...
class GPUEngine:
    ...
    def evaluate_chains_for_ga(self, chains):
        ...
        for ci in range(0, len(chains), cbs):
            cb = chains[ci:ci+cbs]
            seqs, depths = self._compute_chain_seqs(cb)
            # _compute_chain_seqs silently truncates any chain longer than maxd
            # tokens before encoding it for the kernel. The kernel then renders
            # its hit-rule string from that truncated token sequence, so a
            # truncated chain's hits come back keyed by the *truncated* string,
            # not the original `c` from `cb`. Build a truncated->original map so
            # we can translate those keys back before merging into hit_words —
            # otherwise the caller's hit_words dict (keyed by the original,
            # untruncated chain strings) has no matching key and a later
            # `hit_words[rule_str] |= idx_set` in _eval_on_engine raises KeyError.
            trunc_map = {}
            for c in cb:
                parts = c.split()
                if len(parts) > maxd:
                    trunc_map[' '.join(parts[:maxd])] = c
            for wi in range(0, n_words_total, wsb):
                ...
                if found:
                    for local_idx, rule_str in found:
                        orig = trunc_map.get(rule_str, rule_str)
                        hit_words[orig].add(wi + local_idx)
    ...
    def verify_pairs_exact(self, base_words, selected_chains, pairs, batch_size=200000):
        """--exact-recovery: for a small set of (chain_idx, base_word_idx) pairs
        (chain_idx indexes into `selected_chains`, base_word_idx indexes into
        `base_words`), compute the ACTUAL mangled output string on GPU (same
        apply() as the bloom-gated kernels, just unconditional + no bloom) and
        return them aligned with `pairs`, so the caller can check each one
        against a real (non-bloom) set of target_words. Meant to be called
        only with the already-small selected ruleset x its recorded hits —
        NOT a full base_words x rules sweep."""
...
class ProcessEngineProxy:
    ...
    def verify_pairs_exact(self, base_words, selected_chains, pairs, batch_size=200000):
        return self._call('verify_pairs_exact', base_words, selected_chains, pairs,
                           batch_size=batch_size) or [None] * len(pairs)
...
class MultiGPUEngine:
    ...
    def verify_pairs_exact(self, base_words, selected_chains, pairs, batch_size=200000):
        # No sharding: --exact-recovery re-checks only the small (chain, word)
        # pairs already flagged as bloom hits during selection (tens of
        # thousands, not millions), so a single engine handles it fine.
        return self.engines[0].verify_pairs_exact(base_words, selected_chains, pairs,
...
class GPUExtractor:
    ...
    def extract_rules(self, base_words, target_words, **depth_overrides):
        ...
        # STAGE S (build_numeric_seed_families) always explores up to
        # seed_pass_depth (>=9) regardless of --max-depth, since numeric/year
        # seed patterns need that much room. Any candidate deeper than
        # self.user_max_depth that made it into all_counts would otherwise
        # survive into evaluate_full_coverage(), where MAX_CHAIN_DEPTH has
        # since been reset back down to self.user_max_depth — meaning it gets
        # silently truncated there (_compute_chain_seqs) into a *different,
        # shorter* rule than the one actually found here, wasting GPU time
        # scoring a rule string that will never match what Stage S generated,
        # and (via the truncation-map fix above) getting its coverage counted
        # against a rule identity nothing downstream will ever export. Drop
        # over-depth candidates now, at the one choke point every stage's
        # output already passes through, instead of letting them go in only
        # to be cut down silently later.
        over_depth = [r for r in all_counts if len(r.split()) > self.user_max_depth]
        if over_depth:
            log_warn(f"[FILTER] Dropping {len(over_depth)} candidate(s) deeper than "
                      f"--max-depth {self.user_max_depth} (from STAGE S numeric seed families)")
            for r in over_depth:
                del all_counts[r]
...
def main():
    ...
    sel.add_argument('--exact-recovery', action='store_true',
                      help="After selection (--select-mode greedy), run one extra exact-match "
                           "verification pass — no bloom filter, no FPR — checking the selected "
                           "ruleset's actual output strings against a real hash set of "
                           "target_words, and report the TRUE fraction of target_words cracked "
                           "(distinct target entries hit), instead of the base_words-relative "
                           "proxy percentage used during CELF selection. Off by default: it's an "
                           "extra pass (cheap only because the selected ruleset is small "
                           "post-selection, but still real work you may not always want).")
    ...
    if args.select_mode == 'greedy':
        ...
        # Coverage indices from evaluate_full_coverage are indices into
        # base_words (each hit = "this base word, transformed, matches the
        # target bloom filter"), so the covered-mask size and recovery
        # denominator must be len(base_words), not len(target_words).
        n_base = len(base_words)
        ...
        selected, covered_mask = select_rules_by_marginal_coverage(
            coverage, final_counts, n_base, budget=budget, cost_alpha=args.select_cost_alpha)
        ordered_rules = [r for r, _ in selected]
        n_covered = int(covered_mask.sum())
        recovery_pct = n_covered / max(1, n_base)
        sort_desc = f"greedy marginal coverage (CELF, cost_alpha={args.select_cost_alpha})"
        extra_header = [
            f"SELECT mode     : greedy (budget={'unbounded/saturation' if budget is None else budget})",
            f"Target recovery : {n_covered:,}/{n_base:,} ({recovery_pct:.2%})  (base_words-relative proxy)",
        ]
        if args.exact_recovery:
            n_exact, exact_pct, _matched = compute_exact_recovery(
                extractor.gpu_engine, base_words, target_words, selected, coverage)
            extra_header.append(
        ...
        if args.select_budgets:
            ...
            for b in budgets:
                ...
                # matrix (cheap: reuse a fresh boolean mask, no re-eval).
                sub_mask = np.zeros(n_base, dtype=bool)
                for r in sub:
                    c = coverage.get(r)
                    if c is not None and len(c):
                        sub_mask[c] = True
                n_sub_covered = int(sub_mask.sum())
                sub_pct = n_sub_covered / max(1, n_base)
                sub_path = f"{base}.{b}{ext}"
                sub_extra = [
                    f"SELECT mode     : greedy (budget={b}, prefix of full greedy ordering)",
                    f"Target recovery : {n_sub_covered:,}/{n_base:,} ({sub_pct:.2%})",
                ]
        ...
        if args.select_budgets:
            ...
            # expensive), so these prefixes are size-matched only; if you
            # need recovery numbers for frequency-mode prefixes too, run
            # them through evaluate_full_coverage separately (or just diff
            # actual hashcat/john crack counts between the two rulesets,
            # which is presumably the point of the benchmark anyway).
            budgets = sorted({int(b) for b in args.select_budgets.split(',') if b.strip()})
            base, ext = os.path.splitext(args.output)
            ext = ext or '.txt'
            print()
