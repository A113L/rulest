...
> ⚠️ **Careful with `--device all`**
>
>  Mixing a fast NVIDIA GPU with a much slower
> Intel iGPU via `--device all` bottlenecks the whole run down to the slowest
> device's speed.
>
> **Recommendation:** explicitly pick comparable, fast devices instead, e.g.
> `--device 0,1,3`, and leave slower/mismatched GPUs out.
