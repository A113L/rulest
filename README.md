...
The remaining time budget (total `--target-hours` minus Phase 1 duration; Phase 0 and Phase S run outside the time budget) is split evenly across requested depths. User seed rules from `--seed-rules` are extended to deeper depths automatically.
...
Override with `--bloom-mb`. The FPR is logged at startup; a warning is emitted if it exceeds 1 %, suggesting a larger `--bloom-mb` value.
 
### VRAM Management
 
Free VRAM is estimated as **55 %** of total global memory (`VRAM_USAGE_FACTOR = 0.55`). All batch sizes and chain budgets scale proportionally based on this estimate relative to an 8 GB baseline (`vram_scale = min(1.0, vram_gb / 8.0)`, floored at 0.25). The Bloom filter size is set independently by `get_auto_bloom_mb` (see table above) and is not affected by `vram_scale`. A batch floor prevents starvation on very constrained hardware.
...
| Available VRAM | `vram_scale` | Auto bloom size |
|---|---|---|
| < 4 GB | 0.25–0.5× | 64 MB |
| 4–8 GB | 0.5–1.0× | 128 MB |
| ≥ 8 GB | 1.0× (full) | 512 MB |

`vram_scale` applies to batch sizes and chain budgets. Bloom filter size is set independently by `get_auto_bloom_mb` and can always be overridden with `--bloom-mb`.
...
- The combination of `--token-strip`, Phase S seed families, and informed chain generation currently delivers the best results.
- Start with `--max-depth 2` or `--max-depth 6` for the best speed-to-success ratio.
- Rules generated by `rulest_v2.py` are fully GPU-compatible and validated by the built-in `HashcatRuleValidator` before output.

## Known issue: mixing Intel GPUs in multi-GPU mode

- Running `--device all` (or an explicit multi-device list) on a mixed-vendor
  setup — e.g. NVIDIA + Intel — can trigger driver-level stalls that don't
  happen when each GPU is run alone.
- Root cause: some Intel NEO / iGPU driver builds share internal locking
  across vendors within the same process. Recompiling a kernel right after
  concurrent multi-GPU execution (e.g. right after Stage 1 or the Seed Pass)
  can hang the OpenCL build call on the Intel device, even though that same
  device compiles fine completely on its own.
- A Python thread stuck inside a blocking driver call (`clBuildProgram`,
  `clFinish`) cannot be force-killed — only an OS process can be.
- Fix in this version: every GPU now runs in its own isolated OS process
  (see `ProcessEngineProxy` / `_gpu_worker_entry`). Each worker sends a
  heartbeat every 3s; if a device goes silent for more than 480s, the parent
  hard-kills that process and continues the run on the remaining GPU(s)
  instead of hanging indefinitely.
- Single-GPU runs are unaffected — they still use `GPUEngine` directly,
  in-process, with no extra IPC overhead.
- Trade-off: multi-GPU runs now pay a small IPC cost (wordlists, rule sets,
  and the bloom filter are sent to each worker process instead of shared
  in-memory), so raw throughput on a healthy mixed-vendor setup may be
  marginally lower than before — in exchange for the run no longer hanging
  forever when one driver stalls.
- Recommendation: if you consistently see hangs or slowdowns on a mixed
  NVIDIA+Intel setup, prefer `--device 0,1` limited to same-vendor GPUs, or
  run the Intel device in its own separate invocation.
