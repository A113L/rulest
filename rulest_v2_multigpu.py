...
class GPUEngine:
    def __init__(self, params):
        ...
        # Persistent output buffers (found-strings + atomic counter). Re-creating
        # these on every single kernel launch is what made the safety fix for
        # MAX_CHAINS_TO_FIND slow: clCreateBuffer for a multi-MB WRITE_ONLY
        # buffer thousands of times per stage dominates runtime once the buffer
        # can no longer be artificially capped at 5000 slots. We allocate once
        # at the largest size ever requested and reuse it for the engine's
        # lifetime (growing only if a call ever needs more).
        self._fo_buf = None
        self._fo_buf_cap = 0
        self._fc_buf = None
    ...
    def _get_output_buffers(self, mf, outs):
        """Return (fo, fc) sized for at least `outs` slots, reusing the cached
        buffer across calls instead of allocating/freeing every kernel launch.
        In practice `outs` is the same value (capped by MAX_SAFE_RESULTS_PER_BATCH)
        on almost every call within a run, so after the first call this is free."""
        if self._fo_buf is None or self._fo_buf_cap < outs:
            if self._fo_buf is not None:
                try: self._fo_buf.release()
                except: pass
            self._fo_buf = cl.Buffer(self.context, mf.WRITE_ONLY, outs*MAX_CHAIN_STRING_LEN)
            self._fo_buf_cap = outs
        if self._fc_buf is None:
            self._fc_buf = cl.Buffer(self.context, mf.READ_WRITE, 4)
        return self._fo_buf, self._fc_buf
    ...
    def _reset_gpu(self, error):
        ...
        self._fo_buf = None; self._fo_buf_cap = 0; self._fc_buf = None  # tied to old context
    ...
    def _run_single_kernel(self, bd):
        ...
        try:
            ...
            fo, fc = self._get_output_buffers(mf, outs)
    ...
    def _run_chains_against_words(self, base_words, chains, desc="  CHAINS  ", _silent=False,
                                   _shared_combos=None, _shared_lock=None):
        """Shared inner loop: iterate chain batches × word sub-batches, collect matches."""
        counter = Counter()
...
# ----------------------------------------------------------------------
# Multi-GPU engine — distributes word batches across N GPUEngine instances
# ----------------------------------------------------------------------
...
def main():
    ...
    ap.add_argument('--device', default='all', metavar='SPEC',
                    help='Device index, name substring, or "all" (default) to use every GPU in parallel')
